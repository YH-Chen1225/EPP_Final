...
paper. The first table is for power cost function, and the second one is
for exponential cost function. The difference between P\_est1, P\_est2
and P\_est3 are their curvature of utility over piece rate. For P\_est1
and P\_est2, it is pre-defined, which is 1 and 0.88 respectively. For
P\_est3, the curvature of utility over piece rate is regards as a
parameters and curve fit method is applied to find its optimal value,
here is 0.9235.\\
The case for Exp cost function is the same. The Curvature of utility
...
the result from authors. The tables have been covert to graph for easier
...
\textbackslash data file, and it shows following result.
...
for \textbf{minimum distance} method in the paper. As we can see, the
estimation result is identical to the one of authors for power cost
function, and for Exp cost function, the result just slightly different.
For easier comparison reason, please check the graph
\textbf{mini\_dist\_params\_comparison\_exp} and
\textbf{mini\_dist\_params\_comparison\_power} in figures file.
