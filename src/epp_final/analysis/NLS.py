...
#The result similar to the first table in the original code, which compare different method when estimate the benchmarks parameters for power cost function.
...
#This table didn't be created in the original code, and I create it to keep the consistency that comparaing the "power cost function" and exponential
#cost function in the whole project. However, I didn't include the sum of squared error in this table. Following are the reason, 
#first, the sum of squared error for exp cost function seems much larger than the one in power cost function. I am not sure if I estimate it wrong
# so it leads to a large squared error or it is result from the exp cost function per se. Also this table is not include in the origianl code, so 
#I can't compare with it. Nevertheless, the estimation result for each parameters is pretty close to the result show in the paper.
...
def predict_table_three(df):
    be64,se64,be65,se65,bp61,sp61,bp62,sp62,be66,se66,bp63,sp63 = predict_prob(df)
    result3 = predict_table2(be64,se64,be65,se65,bp61,sp61,bp62,sp62,be66,se66,bp63,sp63)
    return result3
...
#Here are using other method to obtain the optimal parameters for both exp and power cost function.
...
def predict_table(bp52_least_squaree,bp52_optt,aut,bp52,sse,sse_aut):
    '''Args:
            bp52_least_squaree : Benchmark parameters for power cost function calculated by least squared method
            bp52_optt : Benchmark parameters for power cost function calculated by Nelder-Mead minimized method
...
def predict_table_r2(be52_ls,be54,be52_optt):
    '''Args:
            be52_ls : Benchmark parameters for exponential cost function calculated by least squared method
            be52_optt : Benchmark parameters for exponential cost function calculated by Nelder-Mead minimized method
            be54 : Benchmark parameters for exponential cost function calculated by curve fit method
    '''
    #Here I also adding the outcome of exponential cost of effort,also adding the authors result for comparasion
...
def predict_table1(be54,be56,se54,se56,bp52,bp53,sp52,se53):
    ...
    Args:
            ...
            se53: standard error for power cost function for other parameters estimation
...
def predict_prob(df):
    ...
    return be64,se64,be65,se65,bp61,sp61,bp62,sp62,be66,se66,bp63,sp63

def predict_table2(be64,se64,be65,se65,bp61,sp61,bp62,sp62,be66,se66,bp63,sp63):
    '''Args:
            be64: Estimation result of parameters of exp cost function when curve = 1, compare to "Table6" of the paper
            se64: Estimation result of standard error of parameters for exp cost function when curve = 1, compare to "Table6" of the paper
            be65: Estimation result of parameters of exp cost function when curve = 0.88, compare to "Table6" of the paper
            se65: Estimation result of standard errors of parameters for exp cost function when curve = 0.88, compare to "Table6" of the paper
            be66: Estimation result of parameters of exp cost function when curve is optimal, compare to "Table6" of the paper
            se66: Estimation result of standard errors of parameters for exp cost function when curve is optimal, compare to "Table6" of the paper
            bp61,sp61,bp62,sp62,bp63,sp63 are defined similarly but with power cost function
    '''
    # Create the dataframe that is similar to table 6 in the paper
