...
def distribution_graph(df, treat1, treat2, treat3, treat4):
    ...
    fig = px.ecdf(
        nwdf, x="buttonpresses", color="treatment", markers=True, marginal="histogram"
    )
    return fig
...
def ci_graph(df):
    ...
    ci_range = (emp_std["buttonpresses"] * 1.96) / np.sqrt(
        emp_count["buttonpresses"] - 1
    )
    fig = go.Figure(
        [
            go.Scatter(
...
def method_comparison(r1, r2):
    ...
    fig.update_layout(
        barmode="group",
        title="Comparison of parameters and SSE",
    )
    return fig
...
def params_comparison(t5, a):
    ...
    trace3 = go.Bar(
        ...
        name="Warm glow coefficient a estimation comparison",
        textposition="auto",
    ...
    trace4 = go.Bar(
        ...
        name="Present bias β estimation comparison",
        textposition="auto",
    ...
    trace5 = go.Bar(
        ...
        name="Discount factor δ estimation comparison",
        textposition="auto",
    ...
    fig.update_layout(
        ...
        f"{a}"
        " "
        "cost function parameters between my result and the result of authors",
    )
    return fig
...
def another_params_comparison(t6, type1, type2, type3):
    ...
    fig.update_layout(
        ...
        f"{type3}"
        " "
        "function parameters between my result and the result of authors",
    )
    return fig
...
def mini_dist(df, a):
    ...
    fig.update_layout(
        barmode="group",
        title="Comparison of minimum dist"
        " "
        f"{a}"
        " "
        "parameters between my result and the result of authors",
    )
    return fig
